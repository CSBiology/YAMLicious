...
let isInt (s:string): bool =
    emitJsStatement (s) """if (typeof $0 != "string") return false // we only process strings!  
                return !isNaN(parseFloat($0)) && // use type coercion to parse the _entirety_ of the string (`parseFloat` alone does not do this)...
                       !isNaN(parseFloat($0)) // ...and ensure strings of whitespace fail"""

let isString (s:string): bool =
    emitJsStatement s """return typeof $0 === 'string' || $0 instanceof String"""
#endif
